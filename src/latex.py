...
def format_row(model, cv, ho, note_cv=None, note_ho=None, note_is_int=False):
...
def generate_table(
    ...
    for dataset, exps in by_dataset.items():
        ...
        if cond:
            ...
            fraction_cv = get(cv, "justified_fraction")
            fraction_ho = get(ho, "justified_fraction")
            lines.append(
                format_row(
    ...
    if any(normal_by_dataset.values()):
        normal_table = generate_table(
            ...
            caption=(
                ...
                "The Note column shows ruleset size for RIPPER and justification fraction for AF-CBA. "
        ...
        for afcba_config, config_label in [("conditional", "cond"), ("global", "glob")]:
            ...
            justified_table = generate_justified_only_table(
                ...
                caption=(
                    f"Classification performance on justified cases only (original datasets), "
                    ...
                    f"subset of cases that AF-CBA could justify. "
    ...
    if any(undersampled_by_dataset.values()):
        ...
        for afcba_config, config_label in [("conditional", "cond"), ("global", "glob")]:
            ...
            justified_table = generate_justified_only_table(
                ...
                afcba_config=afcba_config,
                caption=(
                    f"Classification performance on justified cases only (undersampled datasets), "
                    f"using the {config_name} AF-CBA configuration to determine justified cases. "
                    f"This provides a fairer comparison as all models are evaluated on the same "
                    f"subset of cases that AF-CBA could justify. "
                    f"Cross-validation results are reported as $\\mu\\pm\\sigma$; "
