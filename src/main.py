...
def run_consistency_analysis(args):
    ...
    for dataset_name, data in results.items():
        ...
        print(f"  Consistency: {data['consistency_percentage']:.3f}%")
        print(f"  Cases to remove: {to_remove} ({to_remove / total * 100:.1f}%)")
        print(f"  Class balance: {data['class_balance_ratio']:.3f}")
    return results
...
def main(argv=None):
    ...
    experiment_parent.add_argument(
        ...
        type=float,
        default=2.0,
        help="Delta threshold for conditional preferences",
    )
    experiment_parent.add_argument(
        "--epsilon",
        type=float,
        default=0.5,
        help="Epsilon threshold for conditional preferences",
    )
    experiment_parent.add_argument(
        "--min-support",
        type=int,
