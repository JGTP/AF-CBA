...
def analyse_dataset_consistency(processors: list) -> dict:
    ...
    for processor in processors:
        ...
        try:
            ...
            if n_remove > 0:
                print(
                    f"   Consistency: {consistency_pct:.3f}% (remove {n_remove} cases)"
                )
                print(f"   Class balance: {class_balance_ratio:.3f}")
            else:
                print(f"   Consistency: {consistency_pct:.3f}% (perfect)")
                print(f"   Class balance: {class_balance_ratio:.3f}")
...
def print_performance_comparison(results: dict):
    ...
    for _, row in summary_df.iterrows():
        print(
            f"{row['Model']:<15} {row['Accuracy']:<10.3f} {row['F1-Score']:<10.3f} "
            f"{row['MCC']:<8.3f} {row['Justified fraction']:<10.3f}"
        )
