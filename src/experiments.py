    ...
    **heuristic_kwargs,
    ...
    if n_jobs == 1:
        for idx, test_case in tqdm(
            ...
            justified_mask.append(not used_heuristic)
    else:
        def classify_single_case(test_case_idx):
            ...
            return prediction, not used_heuristic
        ...
        justified_mask = [r[1] for r in results]

    predictions = pd.Series(predictions, index=true_outcomes.index)
    justified_mask = pd.Series(justified_mask, index=true_outcomes.index)

    metrics = compute_classification_metrics(true_outcomes, predictions)
    heuristic_count = int((~justified_mask).sum())
    justified_fraction = float(justified_mask.mean())

    justified_only_metrics = None
    if justified_mask.any():
        justified_preds = predictions[justified_mask]
        justified_true = true_outcomes[justified_mask]
        ...
        justified_only_metrics = {
            ...
            "mcc": jm["mcc"],
            "n_cases": int(justified_mask.sum()),
        }

    timeout_count = len(classifier.timeout_cases)
    both_justified_count = len(classifier.both_justified_cases)
    ...
    result = {
        ...
        "justified_cases": len(predictions) - heuristic_count,
        "heuristic_cases": heuristic_count,
        "heuristic": heuristic,
        "justified_only_metrics": justified_only_metrics,
        "justified_indices": true_outcomes.index[justified_mask].tolist(),
    }
    ...
    if both_justified_count > 0:
        result["both_justified_count"] = both_justified_count
...
class Experiment:
    ...
    def _print_configuration_summary(self):
        ...
        if self.runtime_config.conditional:
            ...
            print(f"     - epsilon: {self.runtime_config.epsilon}")
    ...
    def run_full_experiment(
        ...
        evaluate_complexity: bool = False,
    ...
    def _print_cv_summary(self, results: dict):
        print("\n" + "=" * 80)
        ...
        print(
            "{:<20} {:>14} {:>14} {:>14} {:>14}".format(
                ...
                afcba_metrics["mcc"],
                f"just: {afcba_metrics['justified_fraction']}",
            )
        )
    ...
    def _calculate_cv_averages(
        ...
        afcba_metrics = ["accuracy", "f1_score", "mcc", "justified_fraction"]
        afcba_avg = {
            metric: mean_pm_std([r[metric] for r in afcba_results])
    ...
    def _print_holdout_summary(self, results: dict):
        ...
        for model_name, metrics in results["sklearn"].items():
            ...
            if model_name == "RIPPER" and "ruleset_size" in metrics:
                note = f"{metrics['ruleset_size']} rules"
            print(
                "{:<20} {:>12.3f} {:>12.3f} {:>12.3f} {:>14}".format(
                    model_name,
                    metrics["accuracy"],
                    metrics["f1_score"],
                    metrics["mcc"],
        ...
        print(
            "{:<20} {:>12.3f} {:>12.3f} {:>12.3f} {:>14}".format(
                ...
                afcba_metrics["f1_score"],
                afcba_metrics["mcc"],
                f"{afcba_metrics['justified_fraction']:.3f}",
            )
        )
