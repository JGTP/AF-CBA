        ...
        if ruleset and ruleset.rules:
            for rule in ruleset.rules:
                ...
                for cond in rule.conds:
                    unique_features.add(cond.feature)
        avg_rule_length = total_conditions / ruleset_size if ruleset_size > 0 else 0
        results["RIPPER"] = {
            "accuracy": metrics["accuracy"],
            "f1_score": metrics["f1_score"],
            "mcc": metrics["mcc"],
            ...
            "avg_rule_length": round(avg_rule_length, 2),
...
class Experiment:
    ...
    def run_full_experiment(
        ...
        if self.ML_only:
            print(f"\n{'=' * 80}")
            print("ML-only evaluation (skipping AF-CBA)")
            print(f"{'=' * 80}")
            ML_results, _ = evaluate_ML_models(
                self._X_dev,
                self._X_holdout,
                self._y_dev,
                self._y_holdout,
                self.runtime_config.random_state,
                return_predictions=True,
            )
            print("\n" + "=" * 80)
            print("RIPPER Complexity Metrics")
            print("=" * 80)
            if "RIPPER" in ML_results:
                r = ML_results["RIPPER"]
                print(f"  Rules:            {r.get('ruleset_size', '?')}")
                print(f"  Total conditions: {r.get('total_conditions', '?')}")
                print(f"  Avg rule length:  {r.get('avg_rule_length', '?')}")
                print(f"  Unique features:  {r.get('unique_features', '?')}")
            return {"ML": ML_results}
        print(f"\n{'=' * 80}")
    ...
    def _calculate_cv_averages(
        ...
        afcba_metrics = [
            ...
            "mcc",
            "justified_fraction",
            "both_justified_fraction",
            "unjustified_fraction",
        ]
        afcba_avg = {}
        ...
        afcba_justified_only_list = [
            r.get("justified_only_metrics")
            for r in afcba_results
        ...
        if sklearn_justified_only:
            ...
            for model_name in all_justified_models:
                model_metrics = {}
                folds_with_model = [
                    r for r in sklearn_justified_only if r and model_name in r
                ]
                if not folds_with_model:
                    continue
    ...
    def _print_holdout_summary(self, results: dict):
        ...
        for model_name, metrics in results["sklearn"].items():
            note = "--"
            if model_name == "RIPPER" and "ruleset_size" in metrics:
                note = f"{metrics['ruleset_size']}r {metrics.get('total_conditions', '?')}c {metrics.get('unique_features', '?')}f"
            print(
