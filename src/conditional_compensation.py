...
class ConditionalCompensationChecker:
    ...
    def can_compensate(
        self, focus_case: pd.Series, worse_dims: set[str], better_dims: set[str]
    ) -> bool | None:
        ...
        for rule in applicable_rules:
            ...
            if self._check_thresholds(rule, shap_values, worse_dims, better_dims):
                return True
    ...
    def _compute_conditional_shap(self, rule: RuleContext):
        ...
        shap_vals = self.explainer(X_subset)
        squared_shap = np.square(shap_vals.values)
        mean_shap = squared_shap.mean(axis=0)
        self._shap_cache[rule.rule_id] = {
            name: float(imp)
    ...
    def _check_thresholds(
        ...
        if len(better_dims) < 2:
            return True
        corr_matrix = self._corr_cache.get(rule.rule_id)
        max_corr = 0.0
        for d1 in better_dims:
            for d2 in better_dims:
                if d1 != d2 and d1 in corr_matrix.columns and d2 in corr_matrix.columns:
                    corr_val = abs(corr_matrix.loc[d1, d2])
                    if not np.isnan(corr_val):
                        max_corr = max(max_corr, corr_val)
        if max_corr > self.epsilon:
            return False
        return True
